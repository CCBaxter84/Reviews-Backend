...
function createFakeReview() {
  return {
    ...
    photos: createFakePhotos(),
    reported: faker.random.boolean(),
    product_id: faker.random.number()
  };
}

function createFakePhotos() {
  const numOfPhotos = Math.floor(Math.random() * 6)
  let photos = '[';
  for (let i = 0; i < numOfPhotos; i++) {
    photos += JSON.stringify({ id: i + 1, url: faker.image.imageUrl() });
    i < numOfPhotos - 1 ? photos += ',' : photos;
  }

  return photos + ']';
}

let filename = 'reviewsPhotos';
let count = 1;
